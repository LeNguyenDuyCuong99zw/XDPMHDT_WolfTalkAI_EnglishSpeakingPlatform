...
const GrammarExercisesPage: React.FC = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await aiLearningService.generateGrammarExercises(topic, level, count, provider);
      setExerciseSet(data);
    } catch (err) {
    ...
    <div className="exercises-page">
      ...
      <div className="settings-bar">
        <div className="setting-group">
          ...
          <select value={topic} onChange={(e) => setTopic(e.target.value)} className="setting-select">
        ...
        <div className="setting-group">
          ...
          <select value={count} onChange={(e) => setCount(Number(e.target.value))} className="setting-select">
        ...
        <button onClick={handleGenerate} disabled={loading || !topic} className="generate-btn">
      ...
      {exerciseSet && (
        <div className="exercises-container">
          <div className="exercises-header">
            <h2>📝 {exerciseSet.topic} - {exerciseSet.level} Level</h2>
            ...
            {exerciseSet.exercises.map((ex) => (
              <div key={ex.id} className="exercise-card">
                <div className="exercise-header">
                  ...
                  <span className="exercise-type">{ex.type.replace('_', ' ')}</span>
                </div>
                    ...
                    {ex.options.map((opt, i) => (
                      <label key={i} className="option-label">
                        <input
                          ...
                          onChange={() => setAnswers({ ...answers, [ex.id]: opt })}
                ...
                {showResults && (
                  <div className={`result ${answers[ex.id] === ex.correctAnswer ? 'correct' : 'incorrect'}`}>
                    {answers[ex.id] === ex.correctAnswer ? (
                      '✅ Correct!'
                    ) : (
                      <>
                        ❌ Incorrect. Correct answer: <strong>{ex.correctAnswer}</strong>
                      </>
                    )}
